...
for slice in "${selected_slices[@]}"; do
  ...
  cargo_target_dir="$ROOT_DIR/target/player-decoder-videotoolbox-ios"
  cargo_command=(
    cargo
    build
    --manifest-path "$ROOT_DIR/Cargo.toml"
    --target "$rust_target"
    -p player-decoder-videotoolbox
  )
...
if [[ -n "$unexpected_runtime" ]]; then
  echo "iOS player-decoder-videotoolbox must not bundle FFmpeg runtime dylibs:" >&2
  echo "  $unexpected_runtime" >&2
  exit 1
fi

echo
echo "Built iOS player-decoder-videotoolbox plugin libraries into:"
echo "  $OUTPUT_DIR"
