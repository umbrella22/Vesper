...
framework_info_plist() {
  ...
  /usr/libexec/PlistBuddy -c "Add :CFBundleIdentifier string io.github.ikaros.vesper.player.decoder-videotoolbox-plugin" "$output_path"
...
create_framework() {
  ...
  cp "$source_dir/libplayer_decoder_videotoolbox.dylib" "$binary_path"
  install_name_tool -id "@rpath/$FRAMEWORK_BUNDLE/$FRAMEWORK_NAME" "$binary_path"

  printf '%s\n' \
    'void VesperPlayerDecoderVideoToolboxPluginLinkAnchor(void);' \
    >"$framework_dir/Headers/VesperPlayerDecoderVideoToolboxPlugin.h"
  printf '%s\n' \
    'framework module VesperPlayerDecoderVideoToolboxPlugin {' \
    '  umbrella header "VesperPlayerDecoderVideoToolboxPlugin.h"' \
    '  export *' \
    '  module * { export * }' \
    '}' \
...
verify_no_runtime_dylibs() {
  ...
  if [[ -n "$unexpected" ]]; then
    echo "iOS VideoToolbox decoder plugin must not bundle FFmpeg runtime dylibs:" >&2
    echo "  $unexpected" >&2
    exit 1
  fi
}
...
for slice in "${SELECTED_SLICES[@]}"; do
  ...
  if [[ ! -f "$source_dir/libplayer_decoder_videotoolbox.dylib" ]]; then
    echo "Missing VideoToolbox decoder plugin binary for $slice: $source_dir/libplayer_decoder_videotoolbox.dylib" >&2
    exit 1
  fi
