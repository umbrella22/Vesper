...
slice_output_path() {
  case "$1" in
    ios-arm64)
      echo "$OUTPUT_DIR/iphoneos/libvesper_frame_processor_diagnostic.dylib"
      ;;
    ios-simulator-arm64)
      echo "$OUTPUT_DIR/iphonesimulator/$(vesper_ios_slice_rust_target "$1")/libvesper_frame_processor_diagnostic.dylib"
      ;;
    *)
      return 1
      ;;
...
prepare_plugin_binary() {
  local binary_path="$1"
  install_name_tool -id "@rpath/libvesper_frame_processor_diagnostic.dylib" "$binary_path"
}
...
for slice in "${selected_slices[@]}"; do
  ...
  cp "$cargo_target_dir/$rust_target/$PROFILE_DIR/libvesper_frame_processor_diagnostic.dylib" "$output_path"
...
if [[ ${#simulator_slices[@]} -gt 0 ]]; then
  ...
  cp \
    ...
    "$OUTPUT_DIR/iphonesimulator/libvesper_frame_processor_diagnostic.dylib"
  prepare_plugin_binary "$OUTPUT_DIR/iphonesimulator/libvesper_frame_processor_diagnostic.dylib"
fi
