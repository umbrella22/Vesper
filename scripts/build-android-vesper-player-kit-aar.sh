...
if [[ -x "$PROJECT_DIR/gradlew" ]]; then
  exec "$PROJECT_DIR/gradlew" -p "$PROJECT_DIR" ":vesper-player-kit:$MODULE_TASK"
fi

if command -v gradle >/dev/null 2>&1; then
  exec gradle -p "$PROJECT_DIR" ":vesper-player-kit:$MODULE_TASK"
fi
...
Use one of these options:
  1. Open $PROJECT_DIR in Android Studio and run :vesper-player-kit:$MODULE_TASK
