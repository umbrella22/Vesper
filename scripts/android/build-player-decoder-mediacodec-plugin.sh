...
vesper_android_require_cargo_ndk "Android player-decoder-mediacodec plugins"
...
for abi in "${selected_abis[@]}"; do
  cargo_args=(ndk -o "$OUTPUT_DIR" -t "$abi" build -p player-decoder-mediacodec)
...
if [[ -n "$unexpected_runtime" ]]; then
  echo "player-decoder-mediacodec must not bundle FFmpeg runtime libraries:" >&2
  echo "  $unexpected_runtime" >&2
  exit 1
fi

echo
echo "Built Android player-decoder-mediacodec plugin libraries into:"
echo "  $OUTPUT_DIR"
