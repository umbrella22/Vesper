...
final class ContractFixtureTests: XCTestCase {
    ...
    func testSharedPluginDiagnosticsContractKeepsStableFields() throws {
        ...
        let decoderDetails = try XCTUnwrap(decoder["details"] as? [String: Any])
        XCTAssertEqual(decoderDetails["route"] as? String, "sdkManagedNativeFrame")
        ...
        let sourceNormalizerDetails = try XCTUnwrap(sourceNormalizer["details"] as? [String: Any])
        XCTAssertEqual(sourceNormalizerDetails["selectedVideoStreamIndex"] as? String, "0")
        XCTAssertEqual(sourceNormalizerDetails["audioStreamIndex"] as? String, "1")
        XCTAssertEqual(sourceNormalizerDetails["route"] as? String, "sdkManagedNativeFrame")
